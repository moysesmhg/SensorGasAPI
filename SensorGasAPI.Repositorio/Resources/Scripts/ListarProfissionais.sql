 SELECT DISTINCT Imagens.Imagem Imagem
       ...
       LEFT JOIN Municipio
           ...
           WHERE 1 = 1   
             AND ( Pessoa.PrimeiroNomePessoa LIKE @NomeProfissional OR @NomeProfissional = '')
             AND ( ClasseProfissional.CodigoClasseProfissional =  @CodigoClasseProfissional OR @CodigoClasseProfissional IS NULL )
             AND ( EspecialidadeProfissional.CodigoEspecialidadeProfissional = @CodigoEspecialidadeProfissional OR @CodigoEspecialidadeProfissional IS NULL )
             AND ( Endereco.CodigoEstado = @CodigoEstado OR @CodigoEstado IS NULL )
             AND ( Endereco.CodigoMunicipio = @CodigoMunicipio OR @CodigoMunicipio IS NULL )